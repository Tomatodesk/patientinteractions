...
def list_konsultationen(q: str = ""):
    with get_db() as db:
        if q:
            rows = db.execute(
                "SELECT * FROM konsultationen WHERE grund LIKE ? OR notizen LIKE ? OR begleitperson LIKE ? ORDER BY id DESC",
                (f"%{q}%", f"%{q}%", f"%{q}%")
            ).fetchall()
        else:
...
def create_konsultation(k: KonsultationIn):
    with get_db() as db:
        cur = db.execute("""
            INSERT INTO konsultationen (datum, alter_j, alter_m, alter_ges, dauer, grund, massnahmen, begleitperson, mh, notizen)
            VALUES (?,?,?,?,?,?,?,?,?,?)
        """, (
            k.datum or datetime.now().isoformat(timespec='minutes'),
            k.alter_j or 0, k.alter_m or 0, k.alter_ges or 0,
            k.dauer,
...
def get_stats():
    ...
    if not entries:
        return {"total": 0, "avg_dauer": 0, "mh_pct": 0, "top_grund": None,
...
def export_csv():
    ...
    writer.writerow(["ID","Datum","Alter (J)","Alter (M)","Alter (Gesamt Mo.)","Dauer (min)","Grund","Maßnahmen","Begleitperson","Migrationshintergrund","Notizen"])
    for e in entries:
        writer.writerow([
            e["id"], e.get("datum",""), e.get("alter_j",0), e.get("alter_m",0),
